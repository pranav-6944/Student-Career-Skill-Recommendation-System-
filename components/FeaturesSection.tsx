...
export const FeaturesSection: React.FC = () => {
  ...
  return (
    <section class="py-24 bg-slate-50 dark:bg-slate-950 transition-colors duration-300 border-t border-slate-200 dark:border-slate-800/80">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div class="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <Badge variant="default" class="px-3.5 py-1">ENGINEERING & ANALYTICS FEATURES</Badge>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
          {features.map((feat, idx) => {
            ...
            return (
              <Card key={idx} class="group p-5 flex flex-col justify-between border-2 border-slate-200 dark:border-slate-800/80 bg-white dark:bg-slate-900/90 rounded-3xl shadow-xl hover:border-indigo-500/50 hover:shadow-2xl transition-all duration-300">
                
                {/* Card Image Container with Inset Margin & Rounded Corners */}
                <div class="h-48 w-full overflow-hidden rounded-2xl relative mb-5 border border-slate-200 dark:border-slate-800">
                  <img
                    src={feat.image}
                    alt={feat.title}
                  ...
                  {/* Icon badge */}
                  <div class={`absolute bottom-3 left-4 w-11 h-11 rounded-2xl bg-gradient-to-br ${feat.color} flex items-center justify-center text-white shadow-lg`}>
                    <Icon class="w-5 h-5" />
                  </div>
                </div>
                ...
                <div class="space-y-3 px-2 pb-2">
                  ...
                  <p class="text-slate-600 dark:text-slate-400 text-xs sm:text-sm leading-relaxed font-normal">
                    {feat.description}
                  </p>
                </div>
